...
The **rstantools** package offers several methods for creating packages; please see `?rstan_package_skeleton` for more information.  Here, we'll use the function `rstan_create_package` to initialise a bare-bones
...
With __roxygen__ documentation enabled, we need to generate the documentation for `lm_stan` and update the `NAMESPACE` so the function is exported, i.e., available to users when the package is installed.  Typically this can be done with the function `roxygen2::roxygenise`, but since the package contains C++ code we must use `devtools::document`:

```{r, eval=FALSE}
devtools::document()
```
```{r, echo=FALSE}
suppressMessages(devtools::document(PATH))
```
