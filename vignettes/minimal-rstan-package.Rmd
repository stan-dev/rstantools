...
located in `inst/stan`.  This comes at the cost of creating a strong dependency
of your package on __rstantools__ itself (i.e., __rstantools__ must be installed
for your package to work).  Setting `auto_config = FALSE` removes this
dependency, at the cost of having to manually synchronize Stan C++ files by
running `rstan_config()` every time a package `.stan` file is added, removed, or
even modified.
