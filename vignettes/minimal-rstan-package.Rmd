...
(e.g., `Title`, `Author`, `Maintainer`, and `Description`, but these also can be set with the `fields` argument to `rstan_create_package`).  However,
...
in `inst/stan/foo.stan` is stored as list element `stanmodels$foo`.
...
When __roxygen2__ documentation is enabled, a top-level package file `R/rstanlm-package.R` is created by
`rstan_create_package()` to import necessary functions for other packages and to set up the package for compiling Stan C++ code:
...
With __roxygen__ documentation enabled, we need to generate the documentation for `lm_stan` and update the `NAMESPACE` so the function is exported, i.e., available to users when the package is installed.  This can be done with the function `roxygen2::roxygenize`.
...
__Note:__ For newer versions of __roxygen__, it is necessary to compile the package C++ code prior to running `roxygen2::roxygenize`.  One way to do this is with the __pkgbuild__ package function `compile_dll`, as above.
...
It is also possible to use `devtools::install(quick=FALSE)` to
install the package. The argument `quick=FALSE` is necessary if you want to recompile the Stan models. Going forward, if you only make a  change to the R code or the documentation, you can set `quick=TRUE` to speed up the process, or use `devtools::load_all()`.
...
* Running `rstan_create_package` with `auto_config = TRUE` (the default value) automatically synchronizes the Stan C++ files with the `.stan` model files located in `inst/stan`.  This comes at the cost of creating a strong dependency of your package on __rstantools__ itself (i.e., __rstantools__ must be installed for your package to work).  Setting `auto_config = FALSE` removes this dependency, at the cost of having to manually synchronize Stan C++ files by running `rstantools::rstan_config` every time a package `.stan` file is added, removed, or even modified.

* The function `rstantools::use_rstan` can be used to add Stan functionality to an existing package, instead of building the package from scratch.
